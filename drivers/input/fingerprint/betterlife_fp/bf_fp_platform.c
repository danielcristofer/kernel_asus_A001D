...
#define BF_IOCTL_TRANS_IC_INFO              _IOWR(BF_IOCTL_MAGIC_NO,  22,uint32_t)
...
//中断处理函数
#else
/* Use kernel native GPIO/EINT functions for Qualcomm platform */
#endif
...
static int bf_main_get_gpio_info (struct bf_device *bf_dev)
{
    ...
    if (node) {
        bf_dev->reset_gpio = of_get_named_gpio(node, "betterlife,gpio_reset", 0);
        if(bf_dev->reset_gpio < 0) {
            BF_LOG("get betterlife,gpio_reset fail:%d", bf_dev->reset_gpio);
            return bf_dev->reset_gpio;
        }

        bf_dev->irq_gpio =  of_get_named_gpio(node, "betterlife,gpio_irq", 0);
        if(bf_dev->irq_gpio < 0) {
            BF_LOG("get betterlife,gpio_irq fail:%d", bf_dev->irq_gpio);
        ...
        BF_LOG("betterlife,gpio_reset:%d, betterlife,gpio_irq:%d, irq_num:%d", bf_dev->reset_gpio, bf_dev->irq_gpio, bf_dev->irq_num);
#ifdef NEED_OPT_POWER_ON2V8
...
{
    int error = 0;
    /* Libera GPIOs previamente requisitados para evitar -EBUSY em re-init */
#ifndef MTK_ANDROID_L
    ...
    /*reset pin*/
    if (gpio_is_valid(bf_dev->reset_gpio)) {
        error = gpio_request_one(bf_dev->reset_gpio, GPIOF_OUT_INIT_HIGH, "bf reset");
        if (error) {
    ...
    /*irq pin*/
    if (gpio_is_valid(bf_dev->irq_gpio)) {
        error = gpio_request_one(bf_dev->irq_gpio, GPIOF_IN, "bf irq_gpio");
        if (error) {
...
#if defined(NEED_OPT_POWER_ON1V8)
out3:
#if defined(NEED_OPT_POWER_ON2V8)
#ifndef MTK_ANDROID_L
    gpio_free(bf_dev->power_2v8_gpio);
#endif
#endif //NEED_OPT_POWER_ON2V8
...
static long bf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    BF_LOG("bf_ioctl: cmd=0x%x, arg=0x%lx", cmd, arg);
    ...
    case BF_IOCTL_INIT_ARGS:
        BF_LOG("BF_IOCTL_INIT_ARGS: stub\n");
        break;
    case BF_IOCTL_GAIN_ADJUST:
        BF_LOG("BF_IOCTL_GAIN_ADJUST: stub\n");
        break;
    ...
    case BF_IOCTL_ISBACKLIGHT:
        ...
        break;
    case BF_IOCTL_DISPALY_STATUS:
        BF_LOG("BF_IOCTL_DISPALY_STATUS: stub, arg=%d\n", (int)arg);
        break;
    case BF_IOCTL_SET_PID:
        g_netlink_port = (int)arg;
        BF_LOG("BF_IOCTL_SET_PID: pid=%d\n", g_netlink_port);
        break;
    ...
    case BF_IOCTL_REGISTER_READ_WRITE:
        BF_LOG("F7-49 RWR_ENTER arg=%lx", arg);
        BF_LOG("BTL:BF_IOCTL_REGISTER_READ_WRITE len=%d", read_write_cmd.len);
        ...
        BF_LOG("F7-49 RWR_COPIED len=%d tx0=%02x tx1=%02x",
               read_write_cmd.len,
               read_write_cmd.data_tx[0],
               read_write_cmd.data_tx[1]);

        if(read_write_cmd.len < 32) {
            BF_LOG("F7-49 RWR_BEFORE_SPI_SEND len=%d", read_write_cmd.len);
            spi_send_cmd(g_bf_dev, read_write_cmd.data_tx, read_write_cmd.data_rx, read_write_cmd.len);
            BF_LOG("F7-49 RWR_AFTER_SPI_SEND rx0=%02x rx1=%02x",
                   read_write_cmd.data_rx[0],
                   read_write_cmd.data_rx[1]);
            ...
            /* Aloca buffer TX do tamanho correto para evitar stack overflow */
            tx_buf = kzalloc(dma_size, GFP_KERNEL);
            if (!tx_buf) {
                error = -ENOMEM;
                BF_LOG("failed to alloc tx_buf\n");
                break;
            }

            /* Copia o comando do userspace para o início do buffer TX */
            memcpy(tx_buf, read_write_cmd.data_tx, sizeof(read_write_cmd.data_tx));
            /* Preenche o restante com 0xFF (protocolo SPI comum para read) */
            if (dma_size > sizeof(read_write_cmd.data_tx))
                memset(tx_buf + sizeof(read_write_cmd.data_tx), 0xff,
                       dma_size - sizeof(read_write_cmd.data_tx));

            /* Limpa o buffer de imagem com o TAMANHO CORRETO */
            memset(g_bf_dev->image_buf, 0xff, dma_size);

            spi_set_dma_en(1);
#ifdef USE_SPI1_4GB_TEST
            spi_dma_exchange(g_bf_dev, read_write_cmd, dma_size);
#else
            spi_send_cmd(g_bf_dev, tx_buf, g_bf_dev->image_buf, dma_size);
#endif
            spi_set_dma_en(0);
    ...
    case BF_IOCTL_INIT_DEVICE:
        BF_LOG("F7_IOCTL_INIT_DEVICE_BEFORE_INIT");
        error = bf_init_dts_and_irq(g_bf_dev);
        BF_LOG("F7_IOCTL_INIT_DEVICE_AFTER_INIT error=%d", error);
    break;
...
static ssize_t bf_read (struct file *filp, char  *buff, size_t count, loff_t *ppos)
{
    ...
    BF_LOG("bf_read: count=%zu, image_buf=%p", count, g_bf_dev->image_buf);
    if (!g_bf_dev->image_buf) {
        BF_LOG("bf_read: image_buf is NULL!");
        return -ENOMEM;
    }
...
#ifdef MTK_ANDROID_L
    mt_eint_registration(bf_dev->irq_num, EINTF_TRIGGER_RISING, bf_eint_handler_l, 1);
...
#else
    BF_LOG("F7_BEFORE_REQUEST_IRQ irq=%d", bf_dev->irq_num);

    status = request_threaded_irq(bf_dev->irq_num, NULL, bf_eint_handler,
                                  IRQ_TYPE_EDGE_RISING | IRQF_ONESHOT,
                                  BF_DEV_NAME, bf_dev);

    BF_LOG("F7_AFTER_REQUEST_IRQ irq=%d status=%d",
       bf_dev->irq_num, status);
#endif
...
static int bf_probe(struct platform_device *pdev)
{
...
#else
    BF_LOG("F7_CALL_INIT_FROM_PROBE");
    ...
    /* Inicializa SPI sempre — essencial para Qualcomm */
    status = bf_spi_init(bf_dev);
    if(status) {
        BF_LOG("bf_spi_init fail:%d", status);
        /* Não falha o probe se SPI ainda não estiver ready, apenas loga */
    }
...
#ifndef MTK_ANDROID_L
static int __init bf_plt_driver_init(void)
{
    int ret;

    BF_LOG("F7_PLATFORM_DRIVER_INIT");

    ret = platform_driver_register(&bf_plt_driver);

    BF_LOG("F7_PLATFORM_DRIVER_REGISTER ret=%d", ret);

    return ret;
}
late_initcall(bf_plt_driver_init);

static void __exit bf_plt_driver_exit(void)
{
    platform_driver_unregister(&bf_plt_driver);
}
module_exit(bf_plt_driver_exit);
#endif
