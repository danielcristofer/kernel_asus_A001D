...
int spi_send_cmd(struct bf_device *bf_dev, u8 *tx, u8 *rx, u16 spilen)
{
    ...
    BF_LOG("F7-49 SPI_SEND_ENTER dev=%p len=%u tx0=%02x tx1=%02x",
           bf_dev->spi,
           spilen,
           tx ? tx[0] : 0,
           tx ? tx[1] : 0);
    ...
    BF_LOG("F7-49 SPI_BEFORE_SYNC");
    ret = spi_sync(bf_dev->spi, &m);
    BF_LOG("F7-49 SPI_AFTER_SYNC ret=%d", ret);
    mutex_unlock(&spi_lock);
    BF_LOG("F7-49 SPI_SEND_EXIT ret=%d", ret);
#endif
    return ret;
}
...
static int bf_spi_probe(struct spi_device *spi)
{
    ...
    /* SEMPRE chama spi_setup, não apenas com BF_REE/COMPATIBLE */
    spi_setup(g_bf_dev->spi);
    spi_set_drvdata(spi, g_bf_dev);

    bf_chip_info();
    BF_LOG("bf_spi_probe: chip_type = 0x%x", g_chip_type);
